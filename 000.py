...
if __name__ == "__main__":
    ...
    with open(f' files_for_print_{date.today()}.txt', "w") as file:
        ...
        for i in range(len(lst_tif)):
            w_l_dpi = check_tiff(lst_tif[i], 'banner')
        ...
        # print(lst_all)
            ...
            print(f'Стоимость:{calculation(w_l_dpi[0] / 100, w_l_dpi[1] / 100, "banner")} руб. ')
            itog = itog + calculation(w_l_dpi[0] / 100, w_l_dpi[1] / 100, "banner")
            ...
            file.write("-" *40 + "\n")
            file.write(f'Стоимость:{calculation(w_l_dpi[0] / 100, w_l_dpi[1] / 100, "banner")} руб. \n\n')
            file.write(f'Итого: {round(itog, 2)} руб.')

    # write_file_txt('TEST', *lst_all)
    print(f'Итого: {round(itog,2)} руб.')
    # file.write(f'Итого: {round(itog,2)} руб.')

# st = '\n'.join(lst_tif)
# write_file_txt('Files_for_print', st)
